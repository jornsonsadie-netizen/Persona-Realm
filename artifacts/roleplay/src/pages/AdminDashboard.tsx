...
export default function AdminDashboard() {
  ...
  const handleFetchAvailable = async () => {
    ...
    setFetchError(null);
    try {
      const res = await fetch("/api/admin/models/available");
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: res.statusText }));
        setFetchError((err as any).error || "Failed to fetch models");
        return;
      }
      const data = await res.json() as { id: string; owned_by?: string }[];
      setAvailableModels(data);
    } catch {
      setFetchError("Network error fetching models");
    } finally {
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <button
              ...
              style={{ background: "rgba(155,89,255,0.15)", border: "1px solid rgba(155,89,255,0.4)", color: "#9b59ff" }}
            >
              {fetchingAvailable ? "Fetching..." : "Fetch Available from Provider"}
            </button>
            {fetchError && <span className="text-xs text-red-400">{fetchError}</span>}
          </div>
          {availableModels.length > 0 && (
            <div className="max-h-48 overflow-y-auto rounded-xl border border-primary/10 divide-y divide-primary/10">
              {availableModels.map(m => (
                <div
                  key={m.id}
                  className="flex items-center justify-between px-4 py-2 hover:bg-primary/5 cursor-pointer transition-colors"
                  onClick={() => { setNewModelId(m.id); setNewModelName(m.id.split("/").pop() || m.id); }}
                >
                  <span className="text-xs font-mono text-muted-foreground">{m.id}</span>
                  {m.owned_by && <span className="text-xs text-muted-foreground/50">{m.owned_by}</span>}
                </div>
              ))}
            </div>
          )}
